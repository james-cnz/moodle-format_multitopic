...
class stateactions extends \core_courseformat\stateactions {
    ...
    public function fmt_section_move(
        ...
        $ids = [ $origin->id ];
        $originsub = $origin;
        while ($sections[$originsub->nextupid] && $sections[$originsub->nextupid]->levelsan > $origin->levelsan) {
            $originsub = $sections[$originsub->nextupid];
            $ids.push($originsub->id);
        }
        ...
        format_multitopic_move_section_to($course, $origin, $destination);
